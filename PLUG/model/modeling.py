...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        if isinstance(vocab_size_or_config_json_file, str):
            ...
            self.max_image_position = max_image_position
        else:
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        #    print(self.layer[0].intermediate.dense.weight, self.layer[0].intermediate.dense.weight.grad, self.layer[0].intermediate.dense.master_weight)
...
class BertForClassification(PreTrainedBertModel):
    def __init__(self, config, num_of_classes):
        ...
        self.bert = BertModel(config)
        self.cls = nn.Linear(config.hidden_size, num_of_classes)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None,  class_label=None, checkpoint_activations=False):
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                   output_all_encoded_layers=False, checkpoint_activations=checkpoint_activations)
        pooled_output = self.dropout(pooled_output)
        class_logits = self.cls(pooled_output)

        if class_label is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-1)
            loss = loss_fct(class_logits, class_label.long())
            return loss
        else:
            return class_logits 
...
class PalmPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super(PalmPredictionHeadTransform, self).__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.transform_act_fn = ACT2FN[config.hidden_act] \
            if isinstance(config.hidden_act, str) else config.hidden_act
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layernorm_epsilon)
...
class PalmLMPredictionHead(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(PalmLMPredictionHead, self).__init__()
        self.transform = BertPredictionHeadTransform(config)
...
class PalmPreTrainingHeads(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(PalmPreTrainingHeads, self).__init__()
...
class BertDecoder(nn.Module):
    ...
    #         all_encoder_layers.append(hidden_states)
    #     return all_encoder_layers
    def forward(self, hidden_states, enc_hidden_states, enc_attn_mask, dec_attn_mask, checkpoint_activations=False, output_all_encoded_layers=False, is_infer=False):
        all_encoder_layers = []
        def custom(start, end):
            def custom_forward(*inputs):
                layers = self.layer[start:end]
        ...
        #pre_enc_hidden = copy.deepcopy(enc_hidden_states)
        ...
        if checkpoint_activations:
            ...
            while l < num_layers:
                ...
                #hidden_states = mpu.checkpoint(custom(l, l+chunk_length), hidden_states, enc_attn_mask*1)
                ...
                #hidden_states = mpu.checkpoint(custom(l, l+chunk_length), hidden_states, dec_attn_mask*1)
                l += chunk_length
            # decoder layers
        else:
            for i,layer_module in enumerate(self.layer):
...
class PalmForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, decode_input_ids=None, position_ids=None, decode_attention_mask=None, lm_labels=None, checkpoint_activations=False, is_infer=False, sequence_output=None, parallel_output=True):
        if sequence_output is None:
            ...
            sequence_output = sequence_output.half()
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        decode_output = self.decoder(self.bert.embeddings, sequence_output, decode_input_ids, position_ids, attention_mask, decode_attention_mask, checkpoint_activations=checkpoint_activations, is_infer=is_infer)
...
class ImageDecode(PreTrainedBertModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config):
        super(ImageDecode, self).__init__(config)
        self.embeddings = ImageEmbeddings(config)
        self.decoder = BertDecoder(config)
        self.apply(self.init_bert_weights)

    def forward(self, sequence_output, decode_input_ids, position_ids=None, enc_attn_mask=None, dec_attn_mask=None, checkpoint_activations=False, is_infer=False):

        extended_attention_mask = enc_attn_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.decoder.parameters()).dtype) # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0

        embedding_output = self.embeddings(decode_input_ids, position_ids=position_ids)
        sequence_output = self.decoder(embedding_output,
                                      sequence_output,
                                      extended_attention_mask,
                                      dec_attn_mask,
...
class ImageEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size)
        self.word_embeddings = mpu.VocabParallelEmbedding(
            config.vae_size, config.hidden_size,
            init_method=normal_init_method(mean=0.0,
                                           std=config.initializer_range))
        self.position_embeddings = nn.Embedding(config.max_image_position, config.hidden_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        embeddings = self.dropout(embeddings)
        return embeddings
class ImageForPreTraining(PreTrainedBertModel):
    """BERT model with pre-training heads.
    This module comprises the BERT model followed by the two pre-training heads:
        - the masked language modeling head, and
        - the next sentence classification head.
    ...
    """
    def __init__(self, config):
        super(ImageForPreTraining, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        self.decoder = ImageDecode(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, decode_input_ids=None, position_ids=None, decode_attention_mask=None, lm_labels=None, checkpoint_activations=False, is_infer=False, parallel_output=True):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                   output_all_encoded_layers=False, checkpoint_activations=checkpoint_activations)
        prediction_scores, seq_relationship_score = self.cls(sequence_output, pooled_output)
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        decode_output = self.decoder(sequence_output, decode_input_ids, position_ids, attention_mask, decode_attention_mask, checkpoint_activations=checkpoint_activations, is_infer=is_infer)
