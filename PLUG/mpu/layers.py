...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        #self.timers('Column linear').start()
        input_parallel = copy_to_model_parallel_region(input_)
        # Matrix multiply.
        output_parallel = F.linear(input_parallel, self.weight, self.bias)
        #self.timers('Column linear').stop()
        #self.timers('Column linear gather').start()
        if self.gather_output:
            # All-gather across the partitions.
            output = gather_from_model_parallel_region(output_parallel)
        else:
            output = output_parallel
        #self.timers('Column linear gather').stop()
        #timer_names = ['Column linear', 'Column linear gather']
        #self.timers.log(names=timer_names)
        return output
...
class RowParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        # Matrix multiply.
        #self.timers('Row linear').start()
        output_parallel = F.linear(input_parallel, self.weight)
        #self.timers('Row linear').stop()
        # All-reduce across all the partitions.
        #self.timers('Row linear reduce').start()
        output_ = reduce_from_model_parallel_region(output_parallel)
        #self.timers('Row linear reduce').stop()
        if self.bias is not None:
            output = output_ + self.bias
        else:
            output = output_
        #timer_names = ['Row linear', 'Row linear reduce']
        #self.timers.log(names=timer_names)
        return output
