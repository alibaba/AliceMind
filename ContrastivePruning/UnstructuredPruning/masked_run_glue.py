...
MODEL_CLASSES = {
    "bert": (BertConfig, TeacherBertForSequenceClassification, BertTokenizer),
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        # HACK: the label2id is different between old and new version
        # Here, we follow the setting in new version in *datasets* library
        # If you find the score is low, please check the label2id of teacher is correct as expected!
    ...
    # HACK: add data_idx
    all_idxs = torch.arange(all_input_ids.size(0))
...
def main():
    ...
    parser.add_argument("--contrastive_temperature", type=float, default=0.01)
    parser.add_argument("--extra_examples", type=int, default=512)
    parser.add_argument("--cl_unsupervised_loss_weight", type=float, default=0.1)
    parser.add_argument("--cl_supervised_loss_weight", type=float, default=2)
    parser.add_argument("--alignrep", type=str, default='mean-pooling')
    parser.add_argument(
        "--use_distill",
        action="store_true",
    )
    ...
    assert args.alignrep in ['mean-pooling', 'cls']
    ...
    model = model_class.from_pretrained(
        ...
        cl_unsupervised_loss_weight=args.cl_unsupervised_loss_weight,
        cl_supervised_loss_weight=args.cl_supervised_loss_weight,
        alignrep=args.alignrep,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        with open('{}/result.json'.format(args.output_dir), 'w') as f:
