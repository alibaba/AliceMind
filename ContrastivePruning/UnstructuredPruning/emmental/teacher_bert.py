...
class TeacherBertForSequenceClassification(BertPreTrainedModel):
    def __init__(self, config, alignrep):
    ...
    def forward(
        ...
        # just for encode example to store in the training dataset
        if encode_example:
...
class TeacherBertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        if encode_example:
            ...
            if self.alignrep == 'mean-pooling':
                result = torch.sum(sequence_output * (attention_mask.unsqueeze(-1) == 1), dim=1) / torch.sum(attention_mask, dim=1, keepdim=True)
            else:
                result = pooled_output
            result = torch.nn.functional.normalize(result, p=2, dim=-1)
            return result # bsz * hidden_state
