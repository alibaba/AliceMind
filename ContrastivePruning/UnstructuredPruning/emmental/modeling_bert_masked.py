...
class MaskedBertForSequenceClassification(MaskedBertPreTrainedModel):
    def __init__(self, config, use_contrastive_loss, contrastive_temperature, extra_examples, cl_unsupervised_loss_weight, cl_supervised_loss_weight, alignrep, ce_loss_weight):
        super().__init__(config)
    ...
    def calculate_contrastive_loss(
        ...
        if idx is not None and global_representations_bank is not None:
            ...
            extra_labels = torch.index_select(global_labels_bank, dim=0, index=extra_idx).to(pooled_output).long() # extra
            ...
            extra_idx = extra_idx.to(pooled_output).long()
            ...
            # mask self
            # contrastive_mask: self to self -> 1，e.g., contrastive_mask[0,0] = contrastive_mask[1, rep_num] = contrastive_mask[2, 2*rep_num] = 1
...
class MaskedBertForQuestionAnswering(MaskedBertPreTrainedModel):
    def __init__(self, config, use_contrastive_loss, contrastive_temperature, extra_examples, cl_unsupervised_loss_weight, alignrep, ce_loss_weight):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            total_loss *= self.ce_loss_weight

        total_loss += self.calculate_contrastive_loss(idx, self.global_representations_bank_pretrained, all_output, pooled_output, attention_mask)
        total_loss += self.calculate_contrastive_loss(idx, self.global_representations_bank_finetuned, all_output, pooled_output, attention_mask)
        total_loss += self.calculate_contrastive_loss(idx, self.global_representations_bank_snaps, all_output, pooled_output, attention_mask)

        outputs = (total_loss,) + outputs if start_positions is not None and end_positions is not None else outputs
