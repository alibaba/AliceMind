  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="bert", custom_getter=get_custom_getter(compute_type)):
      with tf.variable_scope("embeddings"):
        ...
        self.embedding_output, self.pos_x0_embedding_table, self.pos_y0_embedding_table, self.pos_x1_embedding_table, self.pos_y1_embedding_table, self.bbox_embedding_table = embedding_postprocessor(
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint, partial_load=False):
  ...
  for x in init_vars:
    ...
    if name not in name_to_variable:
      continue
    if partial_load:
        skip_var = any(name.startswith(x) for x in names_to_ignore)
        if skip_var:
            tf.logging.info("skip loading var: '{}'".format(name))
            continue
...
def embedding_postprocessor(input_tensor,
  ...
  return output, pos_x0_embedding_table, pos_y0_embedding_table, pos_x1_embedding_table, pos_y1_embedding_table, bbox_embedding_table
