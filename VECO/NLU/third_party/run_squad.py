...
"""Finetuning VECO model for question-answering on SQuAD."""
...
def main():
  ...
  parser.add_argument("--dev_file", default=None, type=str, help="The path of dev file.")
  parser.add_argument(
  ...
  parser.add_argument("--do_predict", action="store_true", help="Whether to run predicting.")
  parser.add_argument("--predict_test_dataset", action="store_true", help="Whether to predict test set.")
  parser.add_argument(
  ...
  config_class, model_class, tokenizer_class = XLMRobertaConfig, XLMRobertaForQuestionAnswering, XLMRobertaTokenizer
  ...
  # Training
  if args.do_train:
    train_dataset = load_and_cache_examples(args, tokenizer, args.train_file, evaluate=False, output_examples=False, language=args.train_lang, lang2id=lang2id)
  ...
  results = {}
  if args.do_eval and args.local_rank in [-1, 0] and args.dev_file is not None:
    if args.do_train:
    ...
    # Read dev dataset
    dev_dataset = load_and_cache_examples(args, tokenizer, args.dev_file, evaluate=True, output_examples=True,
                              language=lang, lang2id=None, output_original_data=True)
    ...
    for checkpoint in checkpoints:
      ...
      # Evaluate
      result = evaluate(args, dev_dataset, model, tokenizer, prefix=global_step, language=args.eval_lang, lang2id=lang2id)

      result = dict((k + ("_{}".format(global_step) if global_step else ""), v) for k, v in result.items())
      results.update(result)
  if args.do_predict and args.predict_file is not None:
    if args.output_dir is not None and os.path.exists(os.path.join(args.output_dir, 'checkpoint-best')) and args.predict_test_dataset:
    ...
    model.to(args.device)
    if args.predict_test_dataset:
      for language in args.predict_languages.split(','):
        predict_file = os.path.join(args.predict_file, "xquad.{}.json".format(language))
        predict_dataset = load_and_cache_examples(args, tokenizer, predict_file, evaluate=True, output_examples=True,
                                language=language, lang2id=None, output_original_data=False)
        result = evaluate(args, (predict_dataset[0],predict_dataset[1],predict_dataset[2],None), model, tokenizer, prefix="best_checkpoint", language=language, lang2id=lang2id)
    else:
      predict_dataset = load_and_cache_examples(args, tokenizer, args.predict_file, evaluate=True, output_examples=True,
                                language=lang, lang2id=None, output_original_data=False)
      result = evaluate(args, (predict_dataset, None), model, tokenizer, prefix="predict", language=args.eval_lang, lang2id=lang2id)
