...
class SbertForPreTrainingOutput(ModelOutput):
    """
    Output type of :class:`~sofa.utils.BertForPreTraining`.
...
SBERT_START_DOCSTRING = r"""
    ...
    This model inherits from :class:`~sofa.utils.PreTrainedModel`. Check the superclass documentation for the generic
    ...
    Parameters:
        config (:class:`~sofa.models.sbert.SbertConfig`): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~sofa.utils.PreTrainedModel.from_pretrained` method to load the model
            weights.
"""
...
SBERT_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            ...
            Indices can be obtained using :class:`~sofa.models.SbertTokenizer`. See
            :meth:`sofa.utils.PreTrainedTokenizer.encode` and :meth:`sofa.utils.PreTrainedTokenizer.__call__` for
            details.
        ...
        return_dict (:obj:`bool`, `optional`):
            Whether or not to return a :class:`~sofa.utils.file_utils.ModelOutput` instead of a plain tuple.
"""


@dataclass
class BaseModelOutputWithPoolingAndCrossAttentionsWithEmbedding(BaseModelOutputWithPoolingAndCrossAttentions):
...
class SbertModel(SbertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(SBERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
...
class SbertForPreTraining(SbertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(SBERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=SbertForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
            self,
            input_ids=None,
            attention_mask=None,
        ...
        if not return_dict:
            output = (prediction_scores, seq_relationship_score) + outputs[2:-1]
...
@add_start_docstrings(
    """Sbert Model with a `language modeling` head on top for CLM fine-tuning. """, SBERT_START_DOCSTRING
)
class SbertLMHeadModel(SbertPreTrainedModel):
    ...
    def __init__(self, config: SbertConfig):
        ...
        if not config.is_decoder:
            logger.warning("If you want to use `SbertLMHeadModel` as a standalone, add `is_decoder=True.`")
...
@add_start_docstrings("""Sbert Model with a `language modeling` head on top. """, SBERT_START_DOCSTRING)
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.config.adv_grad_factor is not None and self.training:
                    loss = compute_adv_loss(embedding=embedding_output, model=self._forward_call,
                                            ori_logits=logits,
                                            ori_loss=loss,
...
class SbertForMultipleChoice(SbertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(SBERT_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
...
class SbertForQuestionAnswering(SbertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self._forward_call(input_ids=input_ids,
                                     ...
                                     output_hidden_states=output_hidden_states,
                                     return_dict=return_dict)
        return self.compute_loss(outputs, start_positions, end_positions, **outputs.kwargs)
    ...
    def compute_loss(self,
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if self.config.adv_grad_factor is not None and self.training:
                total_loss = compute_adv_loss_pair(embedding=embedding_output, model=self._forward_call,
