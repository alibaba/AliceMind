...
from sofa.utils import mpu, cached_path
import copy
...
class BertForClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,  class_label=None, checkpoint_activations=False, detach_index=-1, pruning_threshold=None):
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                     output_all_encoded_layers=False, checkpoint_activations=checkpoint_activations, detach_index=detach_index,
                                     pruning_threshold=pruning_threshold)
...
class ImageDecode(PreTrainedBertModel):
    ...
    def forward(self, sequence_output, decode_input_ids, position_ids=None, enc_attn_mask=None, dec_attn_mask=None, checkpoint_activations=False, is_infer=False):
        ...
        embedding_output = self.embeddings(decode_input_ids)
        sequence_output = self.decoder(embedding_output,
                                      sequence_output,
                                      extended_attention_mask,
                                      dec_attn_mask,
